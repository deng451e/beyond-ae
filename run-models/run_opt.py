...
from beyond.modified_models.modify_opt import modify_opt_attention
...
@torch.no_grad()
def inference(model, tokenizer, prompts, max_gen_len=200,enable_modify=False):
    past_key_values = None
...
def main(args):
    ...
    if args.enable_modify:
        ...
        KVCache_manager = KVCacheManager(config,dev_map,1,
                                        ...
                                        max_blk_gpu=2,
                                        head_split_num=4)
        KVCache_manager.set_alpha(0.5)
        KVCache_manager.set_beta(1)
        
        modify_opt_attention(model,KVCache_manager,hybrid_attn)
