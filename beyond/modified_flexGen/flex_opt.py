...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id):
        ...
        self.cpu_ratio = self.policy.cpu_ratio 
        self.gpu_ratio = 1-self.policy.cpu_ratio 
        self.load_len  = 0 
    def set_task(self, task):
        self.task = task
    ...
    def load_cache(self, cache_home, cache_read_buf, i):
        ...
        k_gpu, v_gpu, k_cpu, v_cpu,acc = cache_home.val
        ...
        if self.policy.compress_cache:
            ...
            if self.policy.cpu_cache_compute:
                if (k_gpu.device.device_type == DeviceType.MIXED and
                    k_gpu.data[0][0] is not None):
                    path = 2
                else:
                    path = 1
            else:
        ...
        if path == 0:  # Direct copy
            ...
            indices = (slice(0, self.load_len + i ),slice(0,k_gpu.shape[1]) )
            
            if self.policy.attn_sparsity >= 1.0:
                cache_read_buf.store((
                    k_gpu.smart_copy(dst, indices),
                    v_gpu.smart_copy(dst, indices),
                    k_cpu, v_cpu,
            ...
            indices = (slice(0, self.load_len + i ),slice(0,k_gpu.shape[1]) )
            general_copy(k_buf, indices, v_gpu, indices)

            if self.policy.attn_sparsity >= 1.0:
                general_copy(v_buf, indices, v_gpu, indices)
                cache_read_buf.store(((k_buf, False), (v_buf, False),k_cpu, v_cpu))
            else:
                cache_read_buf.store(((k_buf, False), ((v_gpu, v_buf), False),k_cpu, v_cpu))
            ...
            gpu_k_buf = k_gpu.data[0][0]
            gpu_v_buf = v_gpu.data[0][0]

            # shape: (s, b * n_head, head_dim)
            k_buf, v_buf = dst.next_attention_compute_workspace()
            indices = (slice(0, self.load_len + i ),slice(0,k_gpu.shape[1]) )
            general_copy(k_buf, indices, k_gpu, indices)
            general_copy(v_buf, indices, v_gpu, indices)
            cache_read_buf.store((((gpu_k_buf, k_buf,), False),
                                  ((gpu_v_buf, v_buf,), False),k_cpu, v_cpu))
    ...
    def store_cache(self, cache_home, cache_write_buf, i):
        # shape: (s, b * n_head, head_dim)
        k_gpu, v_gpu, k_cpu, v_cpu, acc = cache_home.val
        k_new, v_new, acc_new = cache_write_buf.pop()
        ...
        if i == 0:  # prefill
            ...
            thre = int(self.cpu_ratio*self.task.prompt_len)
           
            self.load_len = self.task.prompt_len-thre
            k_new_cpu = k_new.data[:thre,:,:]
            v_new_cpu = v_new.data[:thre,:,:]
            acc_cpu  = acc_new.data[:thre,:]
             
             
            k_new.data = k_new.data[thre:,:,:]
            v_new.data = v_new.data[thre:,:,:]
            acc_new.data = acc_new.data[thre:,:]
           
           
            k_new.shape = k_new.data.shape
            v_new.shape = v_new.data.shape
            acc_new.shape = acc_new.data.shape

             
           
            k_cpu, v_cpu = self.init_cpu_cache(k_new_cpu,v_new_cpu,acc_cpu) 
            
            ###################################
            indices = (slice(0, k_new.shape[0]),
                       slice(0, k_new.shape[1]))
           
            general_copy(k_gpu, indices, k_new, None)
            general_copy(v_gpu, indices, v_new, None)
            general_copy(acc, indices, acc_new, None)
            cache_home.val = k_gpu, v_gpu, k_cpu, v_cpu, acc 
            ...
            general_copy(k_gpu, indices, k_new, None)
            general_copy(v_gpu, indices, v_new, None)
            # indices = (slice(0, acc_new.shape[0]),slice(0,acc_new.shape[1]))
          
            # general_copy(acc, indices, acc_new, None)
         
     

    def init_cpu_cache(self,k_new,v_new, A):
        k_new = k_new.permute(1,0,2).contiguous() 
        v_new = v_new.permute(1,0,2).contiguous()  
        A = A.permute(1,0)

         
        keep_nums = torch.sum(A >= (1/self.task.prompt_len)*self.policy.alpha ,dim=-1) 
       
        k_cpu,v_cpu = [],[]
        dim0 = self.config.n_head//self.policy.head_split_num
        bh = k_new.size(0)
        ...
        for st in range(0,bh,dim0):
            ...
            # keep_num = min(keep_num,410)
 
            k,v  = k_new[st:ed,:,:],v_new[st:ed,:,:]
            acc = A[st:ed,:]
            ids = acc.topk(keep_num,dim=1).indices  
            dim0_ids = torch.arange(k.size(0))[:, None]
            dim0_ids = dim0_ids.expand_as(ids)
           
            k = k[dim0_ids, ids].contiguous() .to("cpu",non_blocking=True) 
            v = v[dim0_ids, ids].contiguous() .to("cpu",non_blocking=True) 
            # print(torch.sum(acc[dim0_ids, ids])/dim0)
            k_cpu.append(k)
            v_cpu.append(v)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if i == 0:  # prefill
            ...
            cache_write_buf.store((new_k_cache, new_v_cache,acc))

        else:  # decoding
            

            mask, donate[1] = attention_mask.val.smart_copy(self.attention_compute)
            (k_gpu, donate[12]), (v_gpu, donate[13]), k_cpu, v_cpu,(acc_,_) = cache_read_buf.pop()

            
            h, new_k_cache, new_v_cache,acc = self.compute.mha_gen(h, mask, w_q,
                b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                k_gpu, v_gpu, k_cpu, v_cpu,acc_,  donate, self.policy.attn_sparsity,
                self.policy.compress_cache, self.policy.comp_cache_config)
            cache_write_buf.store((new_k_cache, new_v_cache,acc))
...
def run_flexgen(args):
    ...
    head_dim = opt_config.input_dim//head_num 
    hybrid_attn  = AttnMethods(args.gpu_batch_size, head_num,head_dim)

    gpu = TorchDevice("cuda:0")
    cpu = TorchDevice("cpu")
    gpu.hybrid_attn = hybrid_attn
    cpu.hybrid_attn = hybrid_attn
    ...
    with open(f"results/beyond/{args.model.split('-')[-1]}_{num_prompts}_{prompt_len}_{gen_len}.txt", "w", encoding='utf-8') as file:
        for idx in range(args.gpu_batch_size):
...
def add_parser_arguments(parser):
    ...
    parser.add_argument("--head_split_num", type=int, default=2)
