...
from flexgen.utils import (GB, T, cpu_mem_stats, vector_gather,ValueHolder,
...
class TorchDevice:
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        # beyond 
        num_head, hidden_size, prompt_len, gen_len, gpu_batch_size,cpu_ratio = (
            config.n_head, config.input_dim, task.prompt_len, task.gen_len,
            policy.gpu_batch_size,policy.cpu_ratio)
        
        gpu_len =   gen_len+ prompt_len-int(cpu_ratio*prompt_len )-1
        # s,bh,d
        shape = (gpu_len,   gpu_batch_size * num_head , hidden_size // num_head)
        
        # NOTE: disable pin_memory due to high memory overhead
        pin_memory = False
        
        k_cache_gpu = self.allocate(shape, np.float16, pin_memory=pin_memory)
        v_cache_gpu = self.allocate(shape, np.float16, pin_memory=pin_memory)
        acc = self.allocate(shape[:-1], np.float16, pin_memory=pin_memory)
        k_cache_cpu  = []
        v_cache_cpu  =  []
        return k_cache_gpu, v_cache_gpu,k_cache_cpu,v_cache_cpu,acc
    ...
    def mha(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
        ...
        attn_weights = attn_weights.sum(dim=-2)/s
        ...
        attn_weights = TorchTensor.create_from_torch(attn_weights, self)
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
                w_out, b_out, w_ln, b_ln, n_head, k_cache_gpu, v_cache_gpu,k_cache_cpu, v_cache_cpu, acc,donate,
                attn_sparsity, compress_cache, comp_config,kick=False):
        ...
        q = q.view(b, tgt_s, n_head, head_dim).permute(0,2,1,3).reshape(b * n_head, 1, head_dim)
        k = k.view(b, tgt_s, n_head, head_dim).permute(0,2,1,3).reshape(b * n_head, 1, head_dim)
        v = v.view(b, tgt_s, n_head, head_dim).permute(0,2,1,3).reshape(b * n_head, 1, head_dim)
        
        k_cache_gpu = k_cache_gpu.data.permute(1,0,2)
        v_cache_gpu = v_cache_gpu.data.permute(1,0,2)
        attn_output,A_gpu,_  =  self.hybrid_attn(q,k,v,k_cache_gpu,v_cache_gpu, k_cache_cpu,v_cache_cpu) 
        
        
        value = attn_output.view(b, 1, h)
        ...
        if donate[0]: inputs.delete()
        if donate[1]: attention_mask.delete()
        if kick: acc.data = A_gpu.squeeze()
        
        
        k = k.permute(1,0,2)
        v = v.permute(1,0,2)
       
        k_new = TorchTensor.create_from_torch(k, self)
        v_new = TorchTensor.create_from_torch(v, self)
        
        assert not torch.isnan(value.data).any() 
        return TorchTensor.create_from_torch(value, self), k_new, v_new,acc
